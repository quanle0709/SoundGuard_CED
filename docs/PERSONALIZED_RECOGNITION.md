...
## UrbanSound8K domain-shift gate

The independent follow-up evaluator is `tools/evaluate_personalized_recognition_domain_shift.py`. The completed run is `benchmark_results/personalized_recognition_domain_shift/20260914T202306+0700/`; that output is intentionally Git-ignored because it links to locally retained dataset material. It verified the official UrbanSound8K v1.0 archive (6,023,741,708 bytes; MD5 `9aa69802bbf37fb986f71ec1483a196e`; DOI `10.5281/zenodo.1203745`; CC BY-NC 3.0), all 8,732 metadata-linked WAV files, all 10 classes, and all 10 official folds. Four otherwise preferred candidates were skipped and recorded by the production quality gate (three too quiet and/or clipped development candidates and one too-quiet holdout candidate).

The fixed seed split uses five distinct-`fsID` enrollment clips per profile (`DOG BARK`, `CAR HORN`, and `SIREN`) from folds 1-2. Development uses folds 3-5 (15 prototype-development extras, 30 known calibration clips, and 30 unknowns balanced over the other seven classes); the untouched final split uses folds 6-10 (30 known and 30 similarly balanced unknown clips). Holdout quality checking and embedding began only after the development method was frozen. This is a contract-specific open-set personalization split, not UrbanSound8K's recommended 10-fold cross-validation protocol, so its metrics are not comparable to published UrbanSound8K classifier benchmarks.

Frozen EfficientAT `mn10_as` behavior was deliberately conservative on this domain shift: top-1 identity was correct for 29/30 known clips, but all 30 known clips failed the current 0.08 margin and were rejected. All 30 unknown clips were also rejected, producing 0/30 false accepts, 30/30 false rejects, 0/30 misidentifications, and 30/60 correct open-set decisions. Development-only `mean5`, `medoid5`, `trimmed5`, `mean10`, and `trimmed10` candidates reached 47-49/60 development decisions with 0/30 false accepts and 11-13/30 false rejects. None could strictly reduce the frozen method's already-zero false-accept count, so no candidate passed the predeclared safety gate and production thresholds/prototypes were not changed. This run demonstrates open-set rejection and identity ranking, but it also demonstrates that current sound margins are not usable for these three broad UrbanSound8K classes without target-domain calibration; it is not a production accuracy claim or evidence for recognizing a particular household instance.

Across 165 sound embeddings, wall latency was 84.5 ms median and 211.0 ms p95; the maximum 24.97 s includes cold/lazy initialization. Background preload returned control in 4.5 ms, reported ready after 24.05 s (18.89 s worker model load), suppressed duplicate preload requests, and recorded zero queue drops. The preload worker reported about 314 MB RSS at ready and about 413 MB peak RSS after its first post-ready request. Results can vary with cache state and concurrent host load.

The VIVOS result was carried forward without re-running voice inference: 19/20 known utterances were correctly accepted, 25/25 unfamiliar-speaker utterances were rejected, and the final open-set result remained 44/45 with zero false accepts or misidentifications. Its EfficientAT/WeSpeaker revisions and checksums remain those in the model table above. Dataset-level fusion could not form an accepted sound/voice pair because the frozen sound margin rejected every sound holdout; deterministic fusion behavior remains covered by automated regression tests.
